...
const DEFAULT_TOOLBAR_COMMAND = TOOLBAR_COMMANDS.map(cmd => cmd.id)
...
.native-rich-editor__button--lang.has-translation:not(.is-active) .native-rich-editor__button-icon {
    background: rgba(34, 197, 94, 0.18);
    color: #166534;
}

.native-rich-editor__button--lang.has-translation:not(.is-active):hover .native-rich-editor__button-icon {
...
const SUPPORTED_LANGS = ['es', 'va'];

function getRawExternalValue(attrs) {
    const data = attrs.data;
    const name = attrs.name;

    // Priorizar data[name] como fuente de verdad
    if (data && typeof name === 'string' && Object.prototype.hasOwnProperty.call(data, name)) {
        return data[name];
    }

    // Fallback al comportamiento anterior usando attrs.value
...
function normalizeToTranslations(raw) {
    const translations = {};
    ...
    if (raw == null) {
        return { isMulti: false, translations };
    }

    if (typeof raw === 'string') {
        translations[DEFAULT_LANG] = raw;
        return { isMulti: false, translations };
    }
    ...
    if (typeof raw === 'object') {
        SUPPORTED_LANGS.forEach((lang) => {
            if (typeof raw[lang] === 'string') {
                translations[lang] = raw[lang];
            }
        });

        // Si viene como objeto, siempre lo tratamos como multidioma (aunque esté vacío)
        return { isMulti: true, translations };
    }

    try {
        const str = String(raw);
        translations[DEFAULT_LANG] = str;
        return { isMulti: false, translations };
    } catch (e) {
        return { isMulti: false, translations };
    }
...
function getTextForLang(translations, lang) {
    const requestedLang = lang || DEFAULT_LANG;
    ...
    // Fallback a otro idioma soportado con contenido
...
function getExternalValue(vnode) {
    const attrs = vnode && vnode.attrs ? vnode.attrs : {};
    const state = vnode && vnode.state ? vnode.state : {};

    const raw = getRawExternalValue(attrs);

    // Si es string, siempre devolverlo tal cual (modo simple)
    if (typeof raw === 'string') {
        state.isMultiLang = false;
        return raw;
    }

    // Si es objeto, tratamos como multidioma estricto: solo el idioma actual o ""
    if (raw && typeof raw === 'object') {
        state.isMultiLang = true;
        const currentLang = state.currentLang || DEFAULT_LANG;
        const normalized = normalizeToTranslations(raw);
        return typeof normalized.translations[currentLang] === 'string'
            ? normalized.translations[currentLang]
            : '';
    }

    // Null/undefined u otros tipos: normalizar a string simple
    const normalized = normalizeToTranslations(raw);
    state.isMultiLang = normalized.isMulti;
    return getTextForLang(normalized.translations, state.currentLang || DEFAULT_LANG);
}
    ...
    if (typeof document !== 'undefined') {
        ...
        const cleanElement = (element) => {
            ...
            if (element.hasAttribute('style')) {
                ...
                // Si no quedan estilos, eliminar el atributo style (excepto en img para conservar style vacío si el usuario lo añadió)
...
function emitChange(vnode) {
    ...
    let emittedValue = cleaned;

    if (attrs.data && typeof attrs.name === 'string') {
        const data = attrs.data;
        const name = attrs.name;
        const currentLang = state.currentLang || DEFAULT_LANG;
        const isMulti = !!state.isMultiLang;

        if (isMulti) {
            let container = data[name];

            // Si venimos de un string u otro tipo, convertir a objeto de traducciones
            if (container == null || typeof container !== 'object') {
                const prev =
                    typeof container === 'string'
                        ? container
                        : typeof container === 'number' || typeof container === 'boolean'
                            ? String(container)
                            : '';

                const normalized = normalizeToTranslations(prev);
                container = normalized.translations;
            }

            // Asegurar que existen todas las claves soportadas
            SUPPORTED_LANGS.forEach((lang) => {
                if (typeof container[lang] !== 'string') {
                    container[lang] = '';
                }
            });

            container[currentLang] = cleaned;
            data[name] = container;
            emittedValue = container;
        } else {
            data[name] = cleaned;
            emittedValue = cleaned;
        }
    }

    if (typeof attrs.onchange === 'function') {
        attrs.onchange(emittedValue);
    }
}
...
function setupPopoverEditHandlers(vnode, isEditingExisting = false) {
    ...
    const confirmHandler = () => {
        ...
        if (isEditingExisting && popoverLinkNode) {
            ...
            // Crear nuevo enlace: usar selección congelada (pendingLinkSelection) o savedSelection
            const rangeToUse = (state.pendingLinkSelection && !state.pendingLinkSelection.collapsed)
                ? state.pendingLinkSelection.cloneRange()
                : (state.savedSelection && !state.savedSelection.collapsed)
                    ? state.savedSelection.cloneRange()
                    : null;
            state.pendingLinkSelection = null;
            setTimeout(() => {
                const sel = document.getSelection();
                if (rangeToUse && state.editorEl.contains(rangeToUse.commonAncestorContainer) && !rangeToUse.collapsed) {
                    try {
                        ...
                        rangeToUse.insertNode(a);
                        rangeToUse.setStartBefore(a);
                        rangeToUse.setEndAfter(a);
                        if (sel) {
                            sel.removeAllRanges();
                        ...
                        try {
                            ...
                            console.warn('NativeRichEditor: createLink failed', e);
                        }
                    }
                } else {
                    // Sin texto seleccionado: insertar enlace en la posición del cursor
                    restoreSelection(state);
                    try {
                        document.execCommand('createLink', false, safeUrl);
                    } catch (e) {
...
function applyLink(vnode) {
    ...
    // Congelar selección para el próximo enlace si aún no hay pendingLinkSelection
    if (!state.pendingLinkSelection && state.savedSelection && !state.savedSelection.collapsed) {
        state.pendingLinkSelection = state.savedSelection.cloneRange();
    }
...
/**
 * Inserta un string en el editor en la posición actual (cursor/selección).
 * Se usa como callback pasado a attrs.onClickImage.
 * @param {Object} vnode - vnode del componente NativeRichEditor
 * @param {string} str - texto o HTML a insertar (p. ej. URL o <img src="...">)
 */
function insertImageContent(vnode, str) {
    if (typeof str !== 'string') return;
    const { state } = vnode;
    if (state.isSourceView && state.sourceEl) {
        const el = state.sourceEl;
        const start = state.savedSourceStart != null ? state.savedSourceStart : el.selectionStart;
        const end = state.savedSourceEnd != null ? state.savedSourceEnd : el.selectionEnd;
        const value = state.sourceValue || '';
        const newValue = value.slice(0, start) + str + value.slice(end);
        state.sourceValue = newValue;
        el.value = newValue;
        const newPos = start + str.length;
        el.setSelectionRange(newPos, newPos);
        state.savedSourceStart = null;
        state.savedSourceEnd = null;
    ...
    state.editorEl.focus();
    const sel = document.getSelection();
    if (state.pendingImageSelection && sel) {
        sel.removeAllRanges();
        sel.addRange(state.pendingImageSelection.cloneRange());
    } else {
        restoreSelection(state);
    }
    try {
        ...
        console.warn('NativeRichEditor: insertHTML failed', e);
    }
    state.pendingImageSelection = null;
    saveSelection(state);
    updateActiveState(state);
    emitChange(vnode);
}
...
function applyImage(vnode) {
    ...
    if (typeof attrs.onClickImage === 'function') {
        saveSelection(state);
        if (!state.pendingImageSelection && state.savedSelection) {
            state.pendingImageSelection = state.savedSelection.cloneRange();
        }
        if (state.isSourceView && state.sourceEl) {
            state.savedSourceStart = state.sourceEl.selectionStart;
            state.savedSourceEnd = state.sourceEl.selectionEnd;
        } else {
            state.savedSourceStart = null;
            state.savedSourceEnd = null;
        }
        attrs.onClickImage((str) => insertImageContent(vnode, str));
        return;
    }
    if (!state.editorEl || state.isSourceView) {
        return;
    }
    // Fijar posición de inserción si no se congeló ya en mousedown
    if (!state.pendingImageSelection && state.savedSelection) {
        state.pendingImageSelection = state.savedSelection.cloneRange();
    }
    ...
    const handleFileSelect = (event) => {
        ...
        try {
            ...
            // Restaurar la selección congelada o la guardada
            state.editorEl.focus();
            const sel = document.getSelection();
            if (state.pendingImageSelection && sel) {
                sel.removeAllRanges();
...
export const NativeRichEditor = {
    ...
    view: (vnode) => {
        ...
        const hiddenTxtButton = vnode.attrs.hiddenTxtButton || false;
        ...
        return m('div', {
            ...
            m('div', {
                ...
                TOOLBAR_COMMANDS
                    .filter(item => supported_toolbar_commands.includes(item.id))
                    .forEach((item, index) => {
                        ...
                        buttons.push(m('button', {
                            ...
                            onmousedown: (event) => {
                                ...
                                if (sel && sel.rangeCount > 0) {
                                    ...
                                    if (vnode.state.editorEl && vnode.state.editorEl.contains(range.commonAncestorContainer)) {
                                        if (item.action === 'link' && !range.collapsed) {
                                            vnode.state.pendingLinkSelection = range.cloneRange();
                                        }
                                        if (item.action === 'image') {
                                            vnode.state.pendingImageSelection = range.cloneRange();
                                        }
                                        if (!range.collapsed) {
                                            saveSelection(vnode.state);
                                        }
                ...
                // Controles de traducción (selector de idioma y toggle traducible)
                buttons.push(m('div', {
                    key: 'separator-lang',
                    class: 'native-rich-editor__separator'
                }));

                // Botón principal para modo texto simple
                hiddenTxtButton ? null : buttons.push(m('button', {
                    key: 'toggle-multilang',
                    type: 'button',
                    class: `native-rich-editor__button${!isMultiLang ? ' is-active' : ''}`,
                    disabled: disableCollapseToString,
                    title: disableCollapseToString
                        ? 'No se puede volver a texto simple: hay traducciones en varios idiomas'
                        : isMultiLang
                            ? 'Volver a texto simple'
                            : 'Texto simple',
                    onclick: () => {
                        if (disableCollapseToString) {
                            return;
                        }

                        // Solo tiene efecto cuando estamos en modo multi:
                        // colapsa el objeto de traducciones a un string.
                        ...
                        if (data && typeof name === 'string') {
                            ...
                            if (current && typeof current === 'object') {
                                let str = '';
                                if (typeof current.es === 'string' && current.es.trim() !== '') {
                                    str = current.es;
                                } else if (
                                    typeof current[vnode.state.currentLang] === 'string' &&
                                    current[vnode.state.currentLang].trim() !== ''
                                ) {
                                    str = current[vnode.state.currentLang];
                                } else {
                        ...
                        // Actualizar contenido del editor al colapsar a string
                        const nextValue = getExternalValue(vnode);
                        const sanitizedExternal = sanitizeHtml(vnode, nextValue);
                        const normalizedExternal = normalizeHtml(sanitizedExternal);

                        if (vnode.state.editorEl) {
                            vnode.state.editorEl.innerHTML = normalizedExternal;
                        }

                        vnode.state.lastExternalValue = nextValue;
                        vnode.state.lastEmittedValue = nextValue;
                        updateActiveState(vnode.state);
                    }
                }, m('span', { class: 'native-rich-editor__button-icon' }, 'TXT')));

                // Botones de idioma (es / va)
                    SUPPORTED_LANGS
                        .filter(item => supported_langs.includes(item))
                        .forEach((lang) => {
                    const isCurrent = currentLang === lang && isMultiLang;
                    const hasTranslation = hasTranslationForLang(lang);
                    buttons.push(m('button', {
                        key: `lang-${lang}`,
                        type: 'button',
                        class: `native-rich-editor__button native-rich-editor__button--lang${isCurrent ? ' is-active' : ''}${hasTranslation ? ' has-translation' : ''}`,
                        title: `Editar en ${lang.toUpperCase()}`,
                        onclick: () => {
                            const attrs = vnode.attrs || {};
                            const data = attrs.data;
                            const name = attrs.name;

                            // Si estamos en modo simple, activar modo multi al pulsar ES/VA
                            if (!vnode.state.isMultiLang) {
                                if (data && typeof name === 'string') {
                                    const current = data[name];
                                    const raw = current == null ? '' : current;
                                    const normalized = normalizeToTranslations(raw);
                                    data[name] = normalized.translations;
                                }
                                vnode.state.isMultiLang = true;
                            }

                            if (vnode.state.currentLang === lang && vnode.state.isMultiLang) {
                                return;
                            }

                            vnode.state.currentLang = lang;

                            // Actualizar contenido del editor al cambiar de idioma
                            const nextValue = getExternalValue(vnode);
                            const sanitizedExternal = sanitizeHtml(vnode, nextValue);
                            const normalizedExternal = normalizeHtml(sanitizedExternal);
            ...
            m('div', { class: 'native-rich-editor__surface' }, [
                m('div', {
                    ...
                    oncreate: (contentVnode) => {
                        ...
                        // Guardar la selección cada vez que cambie dentro del editor (para enlaces y formato)
                        ...
                        state._selectionChangeCleanup = () => document.removeEventListener('selectionchange', selectionChangeHandler);
                    },
                    onfocus: () => {
